...
//		StartCoroutine(ClearFilename());
	...
	void Update()
	{
		if (Input.GetKeyDown(KeyCode.S))
		{
			...
			m_Recorder.Save();
			m_Progress = 0f;
			if (!string.IsNullOrEmpty(m_LastFile) && (!isRunning))
			{
				StartCoroutine(ClearFilename());
			}
		}

		gifUI.text = "(S)AVE GIF";
		if (m_IsSaving)
		{
			gifUI.text = "SAVING GIF... " + m_Progress.ToString("F2") + "%";
		}
		if (!string.IsNullOrEmpty(m_LastFile))
		{
			gifUI.text += "\nGIF SAVED : <size=5>" + m_LastFile +"</size>";
		}
...
////		if (!string.IsNullOrEmpty(m_LastFile))
////		{
////			gifUI.text += "GIF SAVED : <size=5>" + m_LastFile +"</size>";
////		}
//
...
//	}
	...
	public IEnumerator ClearFilename() {
		isRunning = true;
